        ...
        try {
            axios.post('http://localhost:3002/candidate/verifyOtp', userOtp).then((response) => {
                ...
                if (response.status === 201) {
                    ...
                    history('/registration', { state : { fromOtp: true } });
                }
            }).catch((error) => {
                console.log(error);
                window.alert("Failed to load", error);
            })
        } catch (error) {
            console.log('Error:', error);
            window.alert('Failed to register');
    ...
    return (
        ...
        <div className="w-50 mx-auto" style={{ padding: "20px", margin: "10px" }}>
            ...
            <form onSubmit={handleVerifyOTP} method='post'>
                <div className="row">
                    <div className="mt-4 col-lg-12 d-flex justify-content-center">
                        <input type="text" className="input-enter p-3" name="userotp" id="otp" onChange={handleOTPChange} value={userOtp.userotp} placeholder="Enter OTP" />
                    </div>
                </div>
                <div className="row">
                    <div className="mt-4 col-lg-6 d-flex justify-content-center">
